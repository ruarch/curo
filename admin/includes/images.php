...
<?php if(isset($_GET['action'])){
			$action=$_GET['action'];
			if($action=='add-new'){
  				echo'<h4 style="margin:0px">Gallery Images - Add New</h4>';
			}elseif($action=='edit'){
				echo'<h4 style="margin:0px">Gallery Images - Edit</h4>';
				if(isset($_GET['id'])){
					$id=clean_input($_GET['id']);
					$res=mysql_query("SELECT * FROM cu_images WHERE image_id='$id'") or die(mysql_error());
                  ...
                  <div class="row">
                        ...
                        <div class="span1">
                          ...
                          <select name="image_order" class="span1">
                            <?php @list_image_ordering($image_order);?>
                          </select>
                        </div>
                        
                         <div class="span3">
                         <?php if(isset($_GET['id'])&&$_GET['id']!=''){image_thumbnail(@$image_id); }?>
                        ...
                        <label>Image Gallery:</label>
						 <select name="image_gallery" class="span2">
                            <?php @load_gallery($image_gallery);?>
                          </select>
                        </div>
                  </div>
                  <div class="row">
