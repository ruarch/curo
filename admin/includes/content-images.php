...
<?php if(isset($_GET['action'])){
			$action=$_GET['action'];
			if($action=='add-new'){
  				echo'<h4 style="margin:0px">Content Images - Add New</h4>';
			}elseif($action=='edit'){
				echo'<h4 style="margin:0px">Content Images - Edit</h4>';
				if(isset($_GET['id'])){
...
<?php
	@$action=$_GET['action'];
		@$content_id=clean_input($_GET['conid']);
    ...
    echo '<input type="hidden" name="e_sectionimage" value="'.@$image.'" />';
		echo '<input type="hidden" name="image_con" value="'.@$content_id.'" />';
	
?>
<div class="row" > <div class="span2 pull-right"><input type="submit" name="con_images_btn" class="btn"  value="<?php if(@$action=='add-new'){echo 'Add';}elseif(@$action=='edit'){echo 'Update';}else{echo 'Add';}?>" /></div></div>
                  ...
                  <div class="row">
                        ...
                        <div class="span1">
                         ...
                         <?php if(isset($_GET['id'])&&$_GET['id']!=''){con_section_image_thumbnail(@$image_id); }?>
                          ...
                          <div><?php $rowcon=content_info($content_id); echo $rowcon['content_title'];?></div>
                        </div>
